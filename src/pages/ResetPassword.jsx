...
function ResetPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/user/reset-password/${token}`, {
                newPassword
            });
                ...
                <div className='w-full flex flex-col justify-center'>
                    ...
                    <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
                        <div className='flex flex-col'>
                            <label className='text-sm text-stone-300 mb-1'>New Password</label>
                            <input 
                                type="password" 
                                value={newPassword} 
                                onChange={(e) => {
                        ...
                        <div className='flex flex-col'>
                            <label className='text-sm text-stone-300 mb-1'>Confirm Password</label>
                            <input 
                                type="password" 
                        ...
                        <button 
                            ...
                            className={`mt-4 rounded-md p-3 font-bold text-white transition-all shadow-lg shadow-orange-900/50 
                                ${loading || successMessage ? 'bg-stone-500 cursor-not-allowed' : 'bg-linear-to-r from-orange-600 to-rose-600 hover:scale-[1.02]'}`}
                        >
